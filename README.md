...
- ✅ **Skill terminal action (#152)** — `skill.execute` step supports `action="terminal"` (runs `command` via TerminalTool, `cwd`/`timeout` passthrough) with `capture_to` variable interpolation into subsequent `generate` step prompts (`{capture_to}`); enables self-contained `terminal(fetch) → generate(score) → terminal(publish)` pipelines in one skill
