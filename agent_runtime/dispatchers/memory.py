...
class MemoryDispatcher(SubDispatcher):
    ...
    def _get_memory(self):
        if self._daemon._memory is None:
            from ..memory_engine import MemoryEngine
            self._daemon._memory = MemoryEngine(gateway=self._daemon._gateway)
            logger.info("MemoryEngine created at %s", self._daemon._memory.db_path)
        return self._daemon._memory
