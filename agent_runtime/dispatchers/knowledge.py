...
class KnowledgeDispatcher(SubDispatcher):
    ...
    def _get_kb_manager(self):
        if self._daemon._kb_manager is None:
            from .knowledge_base import KnowledgeBaseManager
            self._daemon._kb_manager = KnowledgeBaseManager()
            logger.info("KnowledgeBaseManager created")
        return self._daemon._kb_manager

    def _get_rag(self):
        if self._daemon._rag is None:
            ...
            self._daemon._rag = RAGPipeline(knowledge_engine=ke, gateway=self._daemon._gateway)
            logger.info(
                "RAGPipeline created (knowledge=%s, gateway=%s)",
                "enabled" if ke else "none",
                "enabled" if self._daemon._gateway._default_client else "stub",
            )
        return self._daemon._rag
