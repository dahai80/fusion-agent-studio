...
class WorkflowDispatcher(SubDispatcher):
    ...
    def _get_workflow_engine(self):
        if self._daemon._workflow_engine is None:
            from ..workflow_engine import WorkflowEngine
            self._daemon._workflow_engine = WorkflowEngine(
                llm_gateway=self._daemon._gateway,
                tool_registry=self._daemon._get_runtime()._tool_registry if self._daemon._runtime else None,
                orchestrator=self._daemon._get_orchestrator(),
            )
            logger.info("WorkflowEngine created")
        return self._daemon._workflow_engine

    def _get_langgraph_engine(self):
        if self._daemon._langgraph_engine is None:
            from ..langgraph_engine import LangGraphEngine
            self._daemon._langgraph_engine = LangGraphEngine()
            logger.info("LangGraphEngine created")
        return self._daemon._langgraph_engine

    def _get_artifact_manager(self):
        if self._daemon._artifact_manager is None:
            from ..artifact_tools import ArtifactManager
            self._daemon._artifact_manager = ArtifactManager()
            logger.info("ArtifactManager created")
        return self._daemon._artifact_manager
